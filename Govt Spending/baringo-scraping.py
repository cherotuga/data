...
class HealthBudgetAnalyzer:
    ...
    def load_and_clean_data(self):
        ...
        print(f"  Approved Budget: {self.approved_budget_col}")
        print(f"  Actual Payments: {self.actual_payments_col}")
    ...
    def _normalize_programme_subtotals(self):
        ...
        programme_subtotal_mask = (
            (pd.notna(self.data[self.programme_col]) & (self.data[self.programme_col].str.strip() != '')) &
            (pd.isna(self.data[self.subprogramme_col]) | (self.data[self.subprogramme_col].str.strip() == ''))
        )
    ...
    def _is_large_administration_entry(self, row):
        ...
        programme = str(row[self.programme_col]).lower() if pd.notna(row[self.programme_col]) else ''
        ...
        is_admin = any(keyword in programme for keyword in admin_keywords)

        if self.debug_is_large_administration_entry:
            print(f"DEBUG _is_large_administration_entry: programme='{programme}', budget={budget_value}, is_admin={is_admin}")
